        ...
        if predictions is None:
            if num_labels:
                self.eval_stats.append(
                    (
                        torch.zeros(0, self.num_ious, dtype=torch.bool),
                        torch.Tensor(),
                        torch.Tensor(),
                        target_cls,
                    )
            ...
            if num_labels:
                ...
                for cls in torch.unique(target_cls_tensor):
                    target_indx = (
                        (cls == target_cls_tensor).nonzero(as_tuple=False).view(-1)
                    )  # target indices
                    pred_indx = (
                        (cls == predictions[:, 5]).nonzero(as_tuple=False).view(-1)
                    )  # prediction indices
                    ...
                    if pred_indx.shape[0]:
                        ...
                        # Appended detections
                        detected_set = set()
                        for iou_indx in (best_ious > self.iou_values[0]).nonzero(
                            as_tuple=False
                        ):
