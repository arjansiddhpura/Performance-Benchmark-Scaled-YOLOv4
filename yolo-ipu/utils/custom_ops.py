...
class Nms(torch.nn.Module):
    ...
    def cpu_nms(
        ...
        cpu_classes = torch.full(
            (batch, max_detections), torch.iinfo(torch.int32).max, dtype=int
        )
    ...
    def forward(
        ...
        return poptorch.custom_op(
            ...
            example_outputs=[
                ...
                torch.zeros(
                    dtype=boxes.dtype, size=[batch, self.nms_max_detections, 4]
                ),
