...
from poptorch import DataLoader, DataLoaderMode, inferenceModel, trainingModel
from poptorch.optim import SGD, Optimizer
...
def ipu_options(
    ...
    model: Detector,
    mode: str,
    opt: argparse.ArgumentParser = None,
):
    """Configure the IPU options using cfg and opt options.
    Parameters:
    ...
    ipu_opts.replicationFactor(
        cfg.system.num_ipus // (1 + len(cfg.model.pipeline_splits))
    )
    ...
    if cfg.ipuopts.available_memory_proportion:
        ...
        if isinstance(amp, float):
            ...
            raise TypeError(
                "Wrong type of cfg.ipuopts.available_memory_proportion. "
    ...
    if mode == "train":
        ...
        ipu_opts.Training.setAutomaticLossScaling(
            enabled=cfg.training.auto_loss_scaling
        )
...
def get_loader(
    ...
    if cfg.model.ipu:
        ...
        loader = torchDataLoader(
            dataset, batch_size=cfg.model.micro_batch_size, shuffle=shuffle
        )
...
def get_optimizer(cfg: yacs.config.CfgNode, model: Detector):
    ...
    weight_decay = (
        ...
        * cfg.ipuopts.gradient_accumulation
        / nominal_batch_size
    )
    ...
    if cfg.model.ipu:
        ...
        optimizer = SGD(
            param_group0, lr=cfg.training.initial_lr, momentum=cfg.training.momentum
        )
...
def get_model_and_loader(
    opt: argparse.ArgumentParser, cfg: yacs.config.CfgNode, mode: str
):
    ...
    if cfg.model.pipeline_splits:
        ...
        for ipu_idx, split in enumerate(cfg.model.pipeline_splits):
            named_layers[split] = poptorch.BeginBlock(
                ipu_id=ipu_idx + 1, layer_to_call=named_layers[split]
            )
...
def set_warmup_lr_and_momentum(
    ...
    for j, x in enumerate(optimizer.param_groups):
        x["lr"] = np.interp(
            batch_idx,
            xi,
            [0.1 if j == 2 else 0.0, x["initial_lr"] * lr_lambda(epoch + 1)],
        )
        if "momentum" in x:
