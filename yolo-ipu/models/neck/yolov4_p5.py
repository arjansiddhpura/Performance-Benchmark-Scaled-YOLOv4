...
from ..layers import ConvNormAct, ResidualBlock, get_norm
...
class CSPNeck(nn.Module):
    def __init__(
        ...
        self.bottleneck_conv1 = ConvNormAct(
            ch_in, ch_out, 1, 1, activation, norm, num_groups
        )
        ...
        self.res_modules = nn.Sequential(
            *[
                ResidualBlock(
                    ch_out, ch_out, activation, norm, num_groups, shortcut=False
                )
...
class CSPDown(nn.Module):
    def __init__(
        ...
        hidden = int(ch_out / 2)
        self.bottleneck_conv = ConvNormAct(
            ch_in, hidden, 3, 2, activation, norm, num_groups
        )
    ...
    def forward(
        self, pan_connection_input: torch.Tensor, x: torch.Tensor
...
class CSPUp(nn.Module):
    def __init__(
        ...
        self.bneck_csp = CSPNeck(
            ch_out * 2, ch_out, num_reps, activation, norm, num_groups
        )
        if upsample:
            self.conv2 = ConvNormAct(
                ch_out, int(ch_out / 2), 1, 1, activation, norm, num_groups
            )
            self.upsample = nn.Upsample(size=None, scale_factor=2, mode="nearest")
        else:
            self.conv2 = ConvNormAct(
                ch_out, ch_out * 2, 3, 1, activation, norm, num_groups
            )
            self.upsample = None
    ...
    def __init__(
        ...
        self.maxp_modules = nn.ModuleList(
            [
                ...
                for kernel in maxp_kernel_size
            ]
        )
        self.maxp_bottleneck_conv = ConvNormAct(
            4 * ch_out, ch_out, 1, 1, activation, norm, num_groups
        )
        self.conv3 = ConvNormAct(ch_out, ch_out, 3, 1, activation, norm, num_groups)

        self.act = activation
        self.norm = get_norm(norm, ch_out * 2, num_groups)
        self.cat_bottleneck = ConvNormAct(
            2 * ch_out, ch_out, 1, 1, activation, norm, num_groups
        )

        self.convup = ConvNormAct(
            ch_out, int(ch_out / 2), 1, 1, activation, norm, num_groups
        )
        self.upsample = nn.Upsample(size=None, scale_factor=2, mode="nearest")
...
class Yolov4P5Neck(nn.Module):
    ...
    def __init__(
        self, activation: nn.Module, norm: str = "group", number_groups: int = 2
    ) -> None:
        super().__init__()

        self.SPP = CSPSPP(
            1024, 512, 1, activation, norm, number_groups, maxp_kernel_size=(5, 9, 13)
        )
        self.cspUp1 = CSPUp(512, 256, 3, activation, norm, number_groups)
        self.cspUp2 = CSPUp(
            256, 128, 3, activation, norm, number_groups, upsample=False
        )
