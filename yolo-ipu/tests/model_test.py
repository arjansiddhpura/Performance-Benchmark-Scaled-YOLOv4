...
class TestYolov4P5:
    ...
    def test_model_and_nms_standalone(self):
        ...
        just_nms_ipu = poptorch.inferenceModel(
    ...
    def test_model_and_nms_end_to_end(self):
        ...
        nms_output_ipu, predictions_just_fwd_ipu = fwd_pass_and_nms_ipu(
        ...
        nms_output_cpu, predictions_just_fwd_cpu = fwd_pass_and_nms_cpu(
            self.transformed_images
        )

        assert (
            torch.max(
                torch.abs(predictions_just_fwd_cpu[0] - predictions_just_fwd_ipu[0])
            )
            <= 1e-3
        )
        assert (
            torch.max(
                torch.abs(predictions_just_fwd_cpu[1] - predictions_just_fwd_ipu[1])
            )
            <= 1e-3
        )
        assert (
            torch.max(
                torch.abs(predictions_just_fwd_cpu[2] - predictions_just_fwd_ipu[2])
            )
            <= 1e-3
        )

        just_nms_ipu = poptorch.inferenceModel(
            PredictionsPostProcessing(cfg_inference, cpu_mode=False)
        )
    ...
    def test_inference_and_nms(self):
        ...
        (
            ...
            m_ap50_cpu,
            m_ap_cpu,
        ) = post_process_and_eval(cfg, y_cpu, self.image_sizes, self.transformed_labels)
    ...
    def test_inference_cpu_and_ipu(self):
        ...
        expected_output_size = model.output_shape(
