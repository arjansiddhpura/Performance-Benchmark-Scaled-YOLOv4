...
class TestAutoAnchors:
    ...
    def test_best_ratio_metric(self, data_manager):
        best_ratio_in_dim, best = data_manager.auto_anchors.best_ratio_metric(
            data_manager.k_points, data_manager.wh
        )
        assert (best_ratio_in_dim == 1).all()
        assert (best == 1).all()

    def test_metric(self, data_manager):
        bpr, aat = data_manager.auto_anchors.metric(
            data_manager.k_points, data_manager.wh
        )
        assert bpr == 1.0
        assert aat == 12.0
    ...
    def test_fitness(self, data_manager):
        fitness = data_manager.auto_anchors.fitness(
            data_manager.k_points, data_manager.wh
        )
        assert fitness == 1.0
