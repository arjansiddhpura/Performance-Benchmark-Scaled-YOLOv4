...
from poptorch import Options, inferenceModel, trainingModel
from utils.config import get_cfg_defaults
from utils.postprocessing import post_processing
from utils.preprocessing import Pad, ResizeImage, ToTensor
...
def ipu_options(
    opt: argparse.ArgumentParser, cfg: yacs.config.CfgNode, model: Detector
):
...
def get_image_and_label(cfg):
    ...
    label = np.array(
        [
            [39, 0.319508, 0.745573, 0.020516, 0.028479],
...
def prepare_model(cfg, debugging_nms=False):
    ...
    if cfg.model.mode == "test":
        opt.weights = (
            os.environ["PYTORCH_APPS_DETECTION_PATH"] + "/weights/yolov4-p5-sd.pt"
        )
        model.eval()
...
def post_process_and_eval(cfg, y, image_sizes, transformed_labels):
    ...
    for idx, (pruned_preds, processed_labels) in enumerate(
        ...
        stat_recorder.record_eval_stats(
            processed_labels, pruned_preds, image_sizes[idx], "100238", False
        )
