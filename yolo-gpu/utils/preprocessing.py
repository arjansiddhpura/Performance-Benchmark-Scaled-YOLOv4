...
class PadNumpy(object):
    ...
    def __call__(self, item: Tuple[np.array, np.array]) -> Tuple[np.array, np.array]:
        ...
        output_image[
            pad_height : (input_image.shape[0] + pad_height),
            pad_width : (input_image.shape[1] + pad_width),
            :,
        ] = input_image
...
class Pad(object):
    ...
    def __call__(
        self, item: Tuple[Image.Image, np.array]
        ...
        output_image = Image.new(
            input_image.mode, (self.image_size, self.image_size), (114, 114, 114)
        )
...
class ResizeImage(object):
    ...
    def __call__(
        ...
        if ratio_image_size != 1:
            ...
            return (
                image.resize(
                    (int(height * ratio_image_size), int(width * ratio_image_size)),
                    interp,
                ),
                labels,
            )
...
class ToTensor(object):
    ...
    def __call__(
        self, item: Tuple[np.array, np.array]
        ...
        # Ensure the numpy array is writable and contiguous before from_numpy
...
class Mosaic(object):
    ...
    def __call__(
        self, item: Tuple[Tuple[Image.Image, np.array]]
    ) -> Tuple[Image.Image, np.array]:

        # base image of size self.image_size * 2 x self.image_size * 2
        base_img = np.full(
            (self.image_size * 2, self.image_size * 2, self.input_channels), 114
        )
        base_label = []
        ...
        for i, (image, label) in enumerate(item):
            ...
            base_img[ymin_b:ymax_b, xmin_b:xmax_b] = original_image[
                ymin_o:ymax_o, xmin_o:xmax_o
            ]
            padw = xmin_b - xmin_o
            padh = ymin_b - ymin_o
...
class RandomPerspective(object):
    ...
    def __call__(
        ...
        perspective[2, 0] = np.random.uniform(
            -self.perspective, self.perspective
        )  # x perspective (about y)
        perspective[2, 1] = np.random.uniform(
            -self.perspective, self.perspective
        ...
        shear = np.eye(3)
        shear[0, 1] = math.tan(
            np.random.uniform(-self.shear, self.shear) * math.pi / 180
        )  # x shear (deg)
        shear[1, 0] = math.tan(
            np.random.uniform(-self.shear, self.shear) * math.pi / 180
        )  # y shear (deg)
        ...
        translation[1, 2] = (
            np.random.uniform(0.5 - self.translate, 0.5 + self.translate)
            * target_height
        ...
        if (
            (self.border_to_remove[0] != 0)
            or (self.border_to_remove[1] != 0)
            or (combined_matrix != np.eye(3)).any()
