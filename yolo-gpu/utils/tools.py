...
class StatRecorder:
    ...
    def __init__(self, cfg: CfgNode, data_path: str, log_wandb: bool = False):
        ...
        coco_metadata = yaml.safe_load(
            open(
                os.environ["PYTORCH_APPS_DETECTION_PATH"]
                + "/"
                + cfg.model.class_name_path
            )
        )
        ...
        if self.log_wandb:
            ...
            if cfg.model.mode == "train":
                ...
                wandb.save(
                    os.environ["PYTORCH_APPS_DETECTION_PATH"] + "/models/loss.py"
                )

    def reset_train_stats(self):
        self.sum_all_loss, self.sum_box_loss, self.sum_obj_loss, self.sum_cls_loss = (
            0.0,
            0.0,
            0.0,
            0.0,
    ...
    run_coco_eval: bool):
        ...
        self.iou_values = self.iou_values.to(device)
        ...
        if predictions is None:
            if num_labels:
                self.eval_stats.append(
                    (
                        torch.zeros(0, self.num_ious, dtype=torch.bool, device=device),
            ...
            bboxes = xywh_to_xyxy(predictions[:, :4]).detach().to(device)
            scores = predictions[:, 4].detach().cpu().numpy()
            class_pred = predictions[:, 5].detach().cpu().numpy()
            ...
            correct = torch.zeros(predictions.shape[0], self.num_ious, dtype=torch.bool, device=device)

            if num_labels:
                detected = []
                target_cls_tensor = torch.tensor(labels[:, 0], device=device)

                # Target boxes
                target_box = xywh_to_xyxy(torch.tensor(labels[:, 1:], device=device))

                # Per target class
                for cls in torch.unique(target_cls_tensor):
                    target_indx = (
                        (cls == target_cls_tensor).nonzero(as_tuple=False).view(-1).to(device)
                    )  # Target indices
                    pred_indx = (
                        (cls == predictions[:, 5]).nonzero(as_tuple=False).view(-1).to(device)
                    )  # Prediction indices

                    # Search for detections
                    if pred_indx.shape[0]:
                        # Prediction to target IoUs
                        best_ious, best_indxs = iou(
                            bboxes[pred_indx, :4], target_box[target_indx]
                        ...
                        for iou_indx in (best_ious > self.iou_values[0]).nonzero(as_tuple=False):
                            ...
                            if detected_target.item() not in detected_set:
                                ...
                                correct[pred_indx[iou_indx]] = (
                                    best_ious[iou_indx] > self.iou_values
                                )  # IoU threshold is 1xn
                                if len(detected) == num_labels:  # All targets located in image
                                    break

            self.eval_stats.append(
                (correct.cpu().detach().numpy(), scores, class_pred, target_cls)
            )
    ...
    def compute_and_print_eval_metrics(self, output_function):
        ...
        precision, recall, f1, mean_precision, mean_recall, m_ap50, m_ap = (
            0.0,
            0.0,
            0.0,
            0.0,
        ...
        if valid_eval:
            ...
            mean_precision, mean_recall, m_ap50, m_ap = (
                precision.mean(),
                recall.mean(),
                ap50.mean(),
                ap.mean(),
        ...
        output_function(s)
        output_function(
            pf % ("all", self.seen, nt.sum(), mean_precision, mean_recall, m_ap50, m_ap)
        )
    ...
    def write_and_eval_coco(self):
        temp_pred_file = "temp_detections_{}.json".format(
            datetime.now().strftime("%Y%m%d_%H%M%S.%f")[:-4]
        )
        ...
        annotation_file = (
            ...
            + "/annotations/"
            + self.cfg.dataset.test.annotation
        )
        try:
    ...
    def record_inference_stats(
        self,
        inference_round_trip_time: Tuple[float, float, float],
        inference_step_time: float,
    ):
    ...
    def record_training_stats(
        ...
        self.loss_items = (
            torch.cat((box_loss, object_loss, class_loss, total_loss))
            / num_img_per_step
        )  # normalize per image
        self.moving_avg_loss = (self.moving_avg_loss * step_idx + self.loss_items) / (
            step_idx + 1
        )
    ...
    def log_train_step(self, optimizer_state, step_idx, output_func):
        if self.log_wandb:
            ...
            mean_losses = {
                self.loss_keys[i]: self.moving_avg_loss.tolist()[i]
    ...
    def logging(self, output_function, run_coco_eval):
        ...
        avg_latency_sec = [x[2] for x in self.inference_times]
        thr_list = self.inference_throughputs

        # Safe statistics in case of empty lists
        if len(avg_latency_sec) > 0:
            avg_latency_ms_mean = float(1000.0 * np.mean(avg_latency_sec))
            avg_latency_ms_min = float(1000.0 * np.min(avg_latency_sec))
            avg_latency_ms_max = float(1000.0 * np.max(avg_latency_sec))
            avg_latency_ms_std = float(1000.0 * np.std(avg_latency_sec))
        else:
            avg_latency_ms_mean = float("nan")
            avg_latency_ms_min = float("nan")
            avg_latency_ms_max = float("nan")
            avg_latency_ms_std = float("nan")

        if len(thr_list) > 0:
            thr_mean = float(np.mean(thr_list))
            thr_min = float(np.min(thr_list))
            thr_max = float(np.max(thr_list))
        ...
        # Prepare CSV in the order:
        # ipu_mode, device-iterations, micro batch size, image size, precision,
        # avg_latency in ms, average inference throughput in fps,
        # min, max, std for avg_latency (ms),
        # min, max, std for average inference throughput (fps)
        device_iterations = self.cfg.ipuopts.device_iterations
        micro_batch_size = self.cfg.model.micro_batch_size
        image_size = self.cfg.model.image_size
        precision = self.cfg.model.precision

        csv_values = [
            str(self.cfg.model.ipu),
            str(device_iterations),
            str(micro_batch_size),
            str(image_size),
            str(precision),
            f"{avg_latency_ms_mean:.3f}",
            f"{thr_mean:.3f}",
            f"{avg_latency_ms_min:.3f}",
            f"{avg_latency_ms_max:.3f}",
            f"{avg_latency_ms_std:.3f}",
            f"{thr_min:.3f}",
            f"{thr_max:.3f}",
            f"{thr_std:.3f}",
        ]
        output_function(",".join(csv_values))
...
class AutoAnchors:
    ...
    def best_ratio_metric(
        self, k_points: Union[torch.Tensor, np.array], width_height: torch.Tensor
    ):
        """
    ...
    def mutate(
        self, population: np.array, mutation_prob: float = 0.9, sigma: float = 0.1
    ):
        """
    ...
    def kmean_anchors(self, labels_wh: np.array):
        ...
        if n_small_objects > 0:
            ...
            print(
                str(n_small_objects)
                + " label boxes are < 3 pixels in width or height out of "
                + str(len(labels_wh))
            )
        ...
        print(
            ...
            % (self.n_anchors, len(labels_wh_filtered))
        )
        ...
        pbar = tqdm(
            range(self.n_generations), desc="Evlolving anchors with Genetic Algorithm"
        )
        for _ in pbar:
            ...
            if new_fitness_value > fitness_value:
                ...
                pbar.desc = (
                    "Evolving anchors with Genetic Algorithm, fitness: {:.3f}".format(
                        fitness_value
                    )
                )
    ...
    def __call__(self) -> List[AnchorBoxes]:
        ...
        scales = np.random.uniform(
            center - shift, center + shift, size=(shapes.shape[0], 1)
        )
        wh_array = np.concatenate(
            [l[:, 3:5] * s for s, l in zip(shapes * scales, self.dataset.labels)]
        )
        wh_tensor = torch.tensor(wh_array).float()
        wh = np.concatenate(
            [l[:, 3:5] * s for s, l in zip(shapes, self.dataset.labels)]
        )

        best_possible_recall, anchors_above_threshold = self.metric(
            self.anchors, wh_tensor
        )
        print(
            "Anchors/Target: {:.3f}, Best Possible Recall: {:.3f}".format(
                anchors_above_threshold, best_possible_recall
            )
        )
        ...
        if best_possible_recall < 0.98:
            ...
            new_best_possible_recall, _ = self.metric(
                new_anchors.reshape(-1, 2), wh_tensor
            )
            ...
            if new_best_possible_recall > best_possible_recall:
                ...
                new_anchors = [
                    AnchorBoxes(
                        widths=new_anchors[0, :, 0], heights=new_anchors[0, :, 1]
                    ),
                    AnchorBoxes(
                        widths=new_anchors[1, :, 0], heights=new_anchors[1, :, 1]
                    ),
                    AnchorBoxes(
                        widths=new_anchors[2, :, 0], heights=new_anchors[2, :, 1]
                    ),
                ]
            else:
                ...
                new_anchors = [
                    AnchorBoxes(
                        widths=self.anchors[0:4, 0], heights=self.anchors[0:4, 1]
                    ),
                    AnchorBoxes(
                        widths=self.anchors[4:8, 0], heights=self.anchors[4:8, 1]
                    ),
                    AnchorBoxes(
                        widths=self.anchors[8:12, 0], heights=self.anchors[8:12, 1]
                    ),
                ]
        else:
...
def intersection(boxes1: np.array, boxes2: np.array) -> np.array:
    ...
    inter_coords = np.clip(
        np.minimum(boxes1[:, None, 2:], boxes2[:, 2:])
        - np.maximum(boxes1[:, None, :2], boxes2[:, :2]),
        a_min=0,
        a_max=None,
    )
...
def bbox_iou(
    ...
    union = (
        (width1 * height1 + torch.finfo(torch.float32).eps) + width2 * height2 - inter
    )
    ...
    convex_diag_squared = (
        ...
        + convex_height * convex_height
        + torch.finfo(torch.float32).eps
    )
    rho_squared = ((b2x1 + b2x2) - (b1x1 + b1x2)) ** 2 / 4 + (
        (b2y1 + b2y2) - (b1y1 + b1y2)
    ) ** 2 / 4
    ...
    if special_iou_type == "diou":
        ...
        raise ValueError(
            "Type {} not supported. Valid options are 'ciou', 'diou' or None".format(
                special_iou_type
            )
        )
...
def ap_per_class(
    ...
    average_precision, precision_array, recall_array = (
        np.zeros(metric_dim),
        np.zeros(metric_dim),
        np.zeros(metric_dim),
    )
    ...
    for cls_indx, cls in enumerate(unique_classes):
        ...
        if num_pos == 0 or num_gt == 0:
            ...
            precision_array[cls_indx] = np.interp(
                -pr_score, -conf[pos_cls], precision[:, 0]
            )  # p at pr_score

            # AP from recall-precision curve
            for j in range(tp.shape[1]):
                average_precision[cls_indx, j] = compute_ap(
