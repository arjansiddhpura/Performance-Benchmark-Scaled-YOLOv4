...
def average_model_weights(cfg: CfgNode) -> torch.nn.Module:
    ...
    def sorting_fn(ckpt_filename):
        ...
        if match:
            ...
            raise ValueError(
                f"Found checkpoint filename with unexpected format: {ckpt_filename}"
            )
    ...
    if len(checkpoints) > 1:
        ...
        print(
            f"Performing weight averaging on the following checkpoints: {checkpoints}, with decay factor {cfg.training.weight_avg_decay}"
        )
        ...
        raise ValueError(
            f"Not enough model checkpoints to perform weight averaging. Found {len(checkpoints)}, expected at least 2."
        )
...
def create_average_fn(weight_avg_exp_decay: float) -> Callable:
    return (
        lambda averaged_model_parameter, model_parameter, num_averaged: weight_avg_exp_decay
        * averaged_model_parameter
        + (1 - weight_avg_exp_decay) * model_parameter
    )
