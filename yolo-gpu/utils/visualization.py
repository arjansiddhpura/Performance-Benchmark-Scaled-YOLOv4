...
def plotting_tool(
    cfg: CfgNode, pruned_preds_batch: List[np.array], orig_img: List[Image.Image]
):
    """
...
def scale_boxes_to_orig(
    boxes: Union[np.array, torch.Tensor],
    ...
    if type(boxes).__module__ == np.__name__:
        ...
        # Ensure tensors are on the same device/dtype as the input boxes
        device = boxes.device
        dtype = xy.dtype
        ratio_image_size = ratio_image_size.to(device=device, dtype=dtype)
        padding = padding.to(device=device)
        xy = xy - padding
        new_boxes = torch.cat((xy, wh), dim=-1) / ratio_image_size
...
def plot_img_bboxes(
    ...
    if image_dir:
        if not os.path.isdir(image_dir):
            os.mkdir(image_dir)
        file_name = "detection_{}.png".format(
