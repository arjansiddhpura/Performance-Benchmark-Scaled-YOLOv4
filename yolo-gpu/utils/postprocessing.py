...
class IPUPredictionsPostProcessing(nn.Module):
    def __init__(
        self,
        inference_cfg: CfgNode,
        cpu_mode: bool,
        testing_preprocessing: bool = False,
    ):
        super().__init__()
    ...
    def indexing_with_batch_size(self, x: torch.Tensor, index: torch.Tensor):
        ...
        if len(x.shape) == 3:
            ...
            # Ensure the batch index tensor is on the same device as x and index
            batch_size_tensor = torch.arange(batch_size, device=x.device).unsqueeze(axis=-1)
            row_index = ((batch_size_tensor * x.shape[1]) + index.to(x.device)).flatten()
            ...
            return torch.gather(x, 1, index.to(x.device))
    ...
    def nms_preprocessing(self, predictions: torch.Tensor) -> Tuple[torch.Tensor, ...]:
        ...
        k = min(self.pre_nms_topk_k, scores.numel())

        # Clamp k to the last-dim size of scores and handle empty
        last_dim = scores.size(-1)
        safe_k = int(min(max(k, 0), last_dim))
        if last_dim == 0 or safe_k == 0:
            # Return empty tensors on the same device/dtype
            empty_scores = scores.new_empty((0,))
            empty_boxes = boxes.new_empty((0, 4))
            empty_classes = classes.new_empty((0,), dtype=torch.long)
            return empty_scores, empty_boxes, empty_classes

        valid_scores, valid_scores_indices = torch.topk(scores, k=safe_k)
        ...
        one_hot_encoded_classes = self.indexing_with_batch_size(
            one_hot_encoded_classes, valid_scores_indices
        )

        # We get new scores which are the multiplication of the class score with the objectness score
        multiplied_scores = (
            one_hot_encoded_classes * valid_scores.unsqueeze(axis=-1)
        ).view(batch_size, -1)

        if self.cpu_mode and not self.testing_preprocessing:
            # We create indices for all the positions in multiplied scores (ensure same device as multiplied_scores)
            all_indices = (
                torch.arange(multiplied_scores.shape[1], device=multiplied_scores.device)
                .unsqueeze(axis=0)
                .repeat(batch_size, 1)
            )
            # We make the indices 0 where the multiplied scores are below self.score_threshold
            masked_indices = (
                all_indices * (multiplied_scores > self.score_threshold)
            ).long()
            ...
            box_indices = self.indexing_with_batch_size(
                valid_scores_indices, box_indices
            )
            ...
            box_shift = (classes * self.max_image_dimension.to(classes.device)).unsqueeze(axis=-1)
    ...
    def nms_postprocessing(
        self, scores: torch.Tensor, boxes: torch.Tensor, classes: torch.Tensor
    ) -> torch.Tensor:
        """
        ...
        return torch.cat(
            (boxes, scores.unsqueeze(axis=-1), classes.unsqueeze(axis=-1).float()),
            axis=-1,
        )
    ...
    def forward(self, predictions: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        """
        scores_pre_nms, boxes_pre_nms, classes_pre_nms = self.nms_preprocessing(
            predictions
        )
...
def post_process_prediction(
    y: torch.Tensor, orig_img_sizes: torch.Tensor, cfg: CfgNode
) -> List[np.array]:
    """
...
def post_process_labels(
    labels: torch.Tensor, orig_img_sizes: torch.Tensor, cfg: CfgNode
) -> List[np.array]:
    """
    ...
    for i, label in enumerate(labels):
        ...
        scaled_boxes = scale_boxes_to_orig(
            label[:, 1:], orig_img_sizes[i], cfg.model.image_size
        )
