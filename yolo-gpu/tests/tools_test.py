...
import numpy as np
import torch
from utils.tools import bbox_iou, ioa, sparse_mean, standardize_labels
...
class TestTools:
    ...
    def test_standardize_labels(self):
        ...
        assert np.array_equal(
            standard_labels,
            np.array(
                [[10.0, width, height, width, height], [14.0, 0.0, 0.0, 0.0, 0.0]]
            ),
        )
    ...
    def test_sparse_mean(self):
        assert sparse_mean(torch.Tensor([1, 2, 3, 0, 0, 0.1, 1e-8])) == torch.Tensor(
            [1.525]
        )
