...
class TestLoss:
    ...
    def test_loss_components(self):
        ...
        predictions = torch.load(
            base_path + "/tests/yolo_loss_input/pred_1268_128.pt", map_location="cpu"
        )
        ...
        predictions = torch.load(
            base_path + "/tests/yolo_loss_input/pred_285_128.pt", map_location="cpu"
        )
    ...
    @pytest.mark.ipus(1)
    @pytest.mark.skip(
        reason="To be enabled for debugging purposes only, test runs for ~30 minutes"
    )
    def test_loss_standalone(self):
        ...
        transformed_images = transformed_images.repeat(
            [cfg.ipuopts.gradient_accumulation, 1, 1, 1]
        )
        transformed_labels = transformed_labels.repeat(
            [cfg.ipuopts.gradient_accumulation, 1, 1]
        )
        ...
        ipu_predictions, (
            ...
            ipu_lobj,
            ipu_lcls,
        ) = ipu_training_model(transformed_images, transformed_labels)
        cpu_total_loss, cpu_lbox, cpu_lobj, cpu_lcls = cpu_training_model.loss(
            ipu_predictions, transformed_labels
        )
    ...
    def test_weight_update(self):
        ...
        for i in range(3):
            _, (total_loss_1, lbox_1, lobj_1, lcls_1) = training_model_1(
                transformed_images, transformed_labels
            )
            total_loss_1.backward()
            optimizer_1.step()

            _, (total_loss_2, lbox_2, lobj_2, lcls_2) = training_model_2(
                transformed_images, transformed_labels
            )
