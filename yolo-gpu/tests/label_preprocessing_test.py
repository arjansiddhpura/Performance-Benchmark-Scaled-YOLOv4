...
class TestYolov4P5Preprocessing:
    ...
    def test_preprocessing(self):
        ...
        real_labels = torch.tensor(
            [
                [45.0000, 0.4795, 0.6416, 0.9556, 0.4466],
                [45.0000, 0.7365, 0.3104, 0.4989, 0.3573],
            ]
        )
        ...
        t_boxes_p5 = torch.round(t_boxes_p5 * 10**n_digits) / (10**n_digits)
        ...
        assert torch.all(
            torch.eq(
                anchor_ind_p4[anchor_ind_p4 != 0.0].sum(),
                torch.tensor([3, 3, 3, 1, 3, 2, 2, 3, 2, 1, 3, 1]).sum(),
            )
        )
        assert torch.all(
            torch.eq(
        ...
        assert torch.all(
            torch.eq(
                ...
                torch.tensor(
                    [
                        [45.0000, 0.1800, 0.6640, 38.2240, 17.8640],
