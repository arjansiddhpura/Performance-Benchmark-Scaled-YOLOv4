...
class TestYolov4Head:
    ...
    anchors = AnchorBoxes(
        widths=torch.tensor(np.array([4, 5, 6]), requires_grad=False),
        heights=torch.tensor(np.array([6, 7, 8]), requires_grad=False),
    )
    input_tensor = torch.Tensor(
        np.random.randn(1, 32, 13, 13).astype(np.float16)
    ).half()
    ...
    def test_inference(self):
        ...
        assert y.shape == torch.Size([1, len(self.anchors.widths) * 13 * 13, 8])
