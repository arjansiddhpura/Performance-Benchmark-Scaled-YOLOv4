...
class TestNms:
    ...
    def prepare_test(self):
        ...
        cpu_preprocessing = IPUPredictionsPostProcessing(
            cfg.inference, cpu_mode=True
        ).nms_preprocessing(predictions)
    ...
    def test_preprocessing_and_nms_standalone(self):
        ...
        ipu_model = IPUPredictionsPostProcessing(
            cfg_inference, cpu_mode=False, testing_preprocessing=True
        )
        cpu_model = IPUPredictionsPostProcessing(
            cfg_inference, cpu_mode=True, testing_preprocessing=True
        )
        ...
        assert torch.all(
            torch.abs(ipu_nms_output[2] - (cpu_nms_output[2] - box_shift)) < 5e-03
        )
    ...
    def test_end_to_end_nms(self):
        ...
        ipu_model = poptorch.inferenceModel(
            IPUPredictionsPostProcessing(cfg_inference, cpu_mode=False)
        )
        ...
        for i, (prediction_ipu, prediction_cpu) in enumerate(
            zip(predictions_ipu, predictions_cpu)
        ):
        ...
        (
            ...
            m_ap50_cpu,
            m_ap_cpu,
        ) = post_process_and_eval(cfg, result_nms_cpu, image_sizes, transformed_labels)
