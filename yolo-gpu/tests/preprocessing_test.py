...
class TestPreprocessing:
    ...
    def test_cutout(self, data_manager):
        ...
        augmented_img, augmented_label = cutout(
            (np.copy(data_manager.img1), np.copy(data_manager.labels))
        )
    ...
    def test_hsv(self, data_manager):
        ...
        augmented_img, augmented_label = hsv(
            (
                Image.fromarray(np.uint8(data_manager.img2)).convert("RGB"),
                np.copy(data_manager.labels),
            )
        )
    ...
    def test_horizontal_flip(self, data_manager):
        ...
        augmented_img, augmented_label = horizontal_flip(
            (
                Image.fromarray(np.uint8(data_manager.img1)).convert("RGB"),
                np.copy(data_manager.labels),
            )
        )
        augmented_img = np.ascontiguousarray(augmented_img)
        ...
        assert np.array_equal(
            augmented_label[..., [0, 2, 3, 4]], data_manager.labels[..., [0, 2, 3, 4]]
        )
        assert np.array_equal(
            augmented_label[..., 1], 1.0 - data_manager.labels[..., 1]
        )
    ...
    def test_vertical_flip(self, data_manager):
        ...
        assert np.array_equal(
            augmented_label[..., [0, 1, 3, 4]], data_manager.labels[..., [0, 1, 3, 4]]
        )
        assert np.array_equal(
            augmented_label[..., 2], 1.0 - data_manager.labels[..., 2]
        )

    def test_to_tensor(self, data_manager):
        to_tensor = ToTensor(max_bbox_per_scale=90, image_type="half")
        augmented_img, augmented_label = to_tensor(
            (np.copy(data_manager.img1), np.copy(data_manager.labels))
        )
    ...
    def test_pad(self, data_manager):
        ...
        assert np.array_equal(
            augmented_label[:, 1],
            (data_manager.labels[:, 1] * data_manager.img1.shape[1] + pad_w)
            / image_size,
        )
        assert np.array_equal(
            augmented_label[:, 2],
            (data_manager.labels[:, 2] * data_manager.img1.shape[0] + pad_h)
            / image_size,
        )
        assert np.array_equal(
            augmented_label[:, [3, 4]],
    ...
    def test_resize_image(self, data_manager):
        ...
        augmented_img, augmented_label = resize_image(
            (
                Image.fromarray(np.uint8(data_manager.img3)).convert("RGB"),
                np.copy(data_manager.labels),
            )
        )
        augmented_img = np.ascontiguousarray(augmented_img)
