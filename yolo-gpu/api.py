...
from utils.preprocessing import Pad, ResizeImage, ToNumpy, ToTensor
...
class YOLOv4InferencePipeline:
    def __init__(self, checkpoint_path: str):
        ...
        self.class_names = yaml.safe_load(open("configs/class_name.yaml"))[
            "class_names"
        ]
    ...
    def __call__(self, image: Image) -> Tuple[List[YoloOutput], List[np.array]]:
        ...
        named_output = [
            YoloOutput._make(obj[0:5] + [obj[-1]] + [self.class_names[int(obj[-1])]])
            for obj in batch_as_list
        ]
    ...
    def preprocess_image(self, img: Image) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        transformed_image, transformed_labels = pad_mthd(
            (transformed_image, transformed_labels)
        )
        transformed_image, transformed_labels = image_to_tensor_mthd(
            (transformed_image, transformed_labels)
        )
        transformed_image, _ = torch.unsqueeze(
            transformed_image, dim=0
        ), torch.unsqueeze(transformed_labels, dim=0)

        return transformed_image, size

    def prepare_inference_yolo_v4_model_for_ipu(self):
        mode = "inference"
    ...
    def plot_img(
        self, processed_batch: List[YoloOutput], original_img: Image
    ) -> List[str]:
