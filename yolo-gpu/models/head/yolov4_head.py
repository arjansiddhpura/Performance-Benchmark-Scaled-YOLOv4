...
class Yolov4Head(nn.Module):
    ...
    def forward(self, feature: torch.Tensor) -> torch.Tensor:
        ...
        x = x.reshape(
            micro_batch_size, len(self.anchors), self.num_outputs, num_rows, num_cols
        )
        ...
        # Make anchor grid/device/dtype match the predictions
        anchor_centers = self.anchor_centers(num_rows, num_cols).to(
            device=point_xy.device, dtype=point_xy.dtype
        )
        two = point_xy.new_tensor(2.0)
        half = point_xy.new_tensor(0.5)
        stride = point_xy.new_tensor(self.stride)
        xy_coord = (anchor_centers + point_xy * two - half) * stride

        # Predicted box width and height
        # width, height = torch.split(
        #     (weight_height * torch.tensor([2.0], dtype=self.precision)), [1, 1], dim=4
        # )

        two_wh = weight_height.new_tensor(2.0)
        width, height = torch.split((weight_height * two_wh), [1, 1], dim=4)
        ...
        final_preds = torch.cat(
            [xy_coord, width, height, class_conf, preds[..., 5:]], dim=-1
        )
