...
class Yolov4P5(Detector):
    ...
    def __init__(
        ...
        if cfg.model.mode == "train":
            ...
            self.ipu_post_process = IPUPredictionsPostProcessing(
                specific_mode_parameters, self.cpu_mode
            )
            self.training = False
        ...
        if self.cpu_mode and cfg.precision != "half":
    ...
    def forward(
        ...
        if self.nms:
            if self.debugging_nms:
                return (
                    self.ipu_post_process(torch.cat(predictions, axis=1)),
                    predictions,
                )
            else:
